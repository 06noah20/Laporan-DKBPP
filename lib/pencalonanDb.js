...
// Senarai kementerian + bilangan calon (untuk navigasi sisi kiri).
// statusFilter: 'diperaku' | 'belum_diperaku' | undefined (semua)
function kementerianCounts(statusFilter) {
  let records = readAll();
  if (statusFilter) records = records.filter((r) => r.status === statusFilter);
  const counts = {};
  records.forEach((r) => {
    counts[r.kementerian] = (counts[r.kementerian] || 0) + 1;
  });
  return Object.keys(counts)
    .sort((a, b) => a.localeCompare(b))
    .map((kementerian) => ({ kementerian, jumlah: counts[kementerian] }));
}
