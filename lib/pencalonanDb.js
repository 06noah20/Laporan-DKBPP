...
const { mapKategori } = require('./extract-pdf');
...
function readAll() {
  ...
  // Keserasian ke belakang: data lama guna 'kategoriPencalonan'.
  records.forEach((r) => {
    if (!r.kategori) r.kategori = mapKategori(r.kategoriPencalonan || r.kategoriRaw || '');
  });
  return records;
}
